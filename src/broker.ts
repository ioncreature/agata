...
export class Broker extends EventEmitter {
  ...
  loadService(name: string): void {
    ...
    service.dependencies.localActions = service
      .getRequiredLocalActions()
      .map(a => localActionName(name, a));
    ...
    function getDependencies(name, dependedBy = []) {
      ...
      singletons[name].getRequiredSingletons().forEach(n => {
        if (dependedBy.includes(name))
          throw new Error(
            `Found singletons circular dependency: ${[...dependedBy, name, n].join(' -> ')}`,
          );
  ...
  getDependencies(): {
    ...
    Object.entries(this.services).forEach(([name, service]) => {
      service
        ...
        .forEach(s => result.singletons[s].dependents.services.push(name));
    ...
    Object.entries(this.singletons).forEach(([name, singleton]) => {
      singleton
        .getRequiredSingletons()
        .forEach(s => result.singletons[s].dependents.singletons.push(name));
    });
    Object.entries(this.plugins).forEach(([name, plugin]) => {
      plugin
        .getRequiredSingletons()
        .forEach(s => result.singletons[s].dependents.plugins.push(name));
    });
    Object.entries(this.actions).forEach(([name, action]) => {
      action.getRequiredActions().forEach(a => result.actions[a].dependents.actions.push(name));
      action
        .getRequiredSingletons()
        .forEach(s => result.singletons[s].dependents.actions.push(name));
  ...
  async mockAction(
    ...
    const loadedDeps = await this.start({
      ...
      plugins: difference(action.getRequiredPlugins(), Object.keys(plugins || {})).reduce(
        (res, plugin) => {
